...
from app.live.rvol_calc import MODERATE_RATIO, STRONG_RATIO
from app.live.session import holiday_for, now_ist
from app.live.types import Snapshot, SymbolRow
from app.services import watchlist_service
from app.tui.confirm_modal import ConfirmModal
from app.tui.rvol_table import RvolTable
from app.tui.watchlist_modal import WatchlistModal
...
class RvolApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical():
            with Horizontal(id="toolbar"):
                ...
                known = any(value == self._watchlist_name for _, value in options)
                yield Select(
                    options,
                    prompt="watchlist",
                    id="watchlist",
                    **({"value": self._watchlist_name} if known else {}),
                )
    ...
    def action_delete_watchlist(self) -> None:
        name = self._watchlist_name
        if not name:
            self._log.append("no saved watchlist selected")
            return
    ...
    def _confirm_delete(self, confirmed: bool | None) -> None:
        ...
        self._log.append(f"deleted '{deleted}'")
    ...
    def check_action(self, action: str, parameters) -> bool:
        ...
        return action not in {
            "cycle_sort", "reverse_sort", "focus_filter", "quit", "toggle_freeze",
            "focus_watchlist", "new_watchlist", "edit_watchlist", "delete_watchlist",
        }
    ...
    def _update_cells(self, rows: dict[str, SymbolRow]) -> None:
        ...
        for key, row in rows.items():
            ...
            for (_, column_key, _, _), value in zip(COLUMNS, cells):
    ...
    def _market_badge(self, snapshot: Snapshot) -> Text:
        ...
        if snapshot.market_status == "NORMAL_OPEN":
            ...
            label = snapshot.market_status.replace("_", " ").title() if snapshot.market_status != "UNKNOWN" else "Closed"
    ...
    def _status(self, snapshot: Snapshot, shown: int) -> Text:
        ...
        if not snapshot.rows:
            ...
            if self._log:
                empty.append("   " + " | ".join(self._log[-2:]), style="yellow")
            return empty
        ...
        status.append(f"sort {SORTABLE[self._sort_index]}{'v' if self._descending else '^'}  ", style="cyan")
        if self._filter:
        ...
        if self._log:
            status.append("   " + " | ".join(self._log[-2:]), style="yellow")
        return status
