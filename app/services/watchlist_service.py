...
def list_watchlists() -> list[dict]:
    ...
    rows = get_connection().execute(
        "SELECT name, symbols_json, updated_at FROM watchlists ORDER BY name"
    ).fetchall()
    return [
        {"name": row["name"], "symbols": json.loads(row["symbols_json"]), "updated_at": row["updated_at"]}
        for row in rows
    ]
...
def save_watchlist(name: str, symbols: list[str]) -> None:
    with transaction() as conn:
        conn.execute(
            ...
            """,
            (name, json.dumps(symbols), dt.datetime.now(dt.timezone.utc).isoformat()),
        )
