...
def fetch_daily_candles_cached(instrument_key: str, lookback_days: int) -> list[DailyBar]:
    ...
    if latest_cached is None:
        bars = ingestion_service.fetch_daily_candles(instrument_key, from_date=from_date, to_date=today)
    ...
    if latest_cached < yesterday:
        ...
        new_bars = ingestion_service.fetch_daily_candles(instrument_key, from_date=delta_from, to_date=today)
...
def fetch_daily_candles_cached_bulk(
    ...
    with ThreadPoolExecutor(max_workers=min(MAX_CONCURRENT_HISTORY_FETCHES, len(instrument_keys))) as pool:
        futures = {
            pool.submit(fetch_daily_candles_cached, key, lookback_days): key for key in instrument_keys
        }
