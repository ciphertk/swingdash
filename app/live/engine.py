...
from app.live import baseline as baseline_store
from app.live import rvol_calc
from app.live.feed import Feed
from app.live.session import Session, active_session, now_ist
from app.live.types import Baseline, Snapshot, SymbolRow
...
class SymbolState:
    ...
    __slots__ = (
        "symbol", "instrument_key", "baseline",
        "vtt", "ltp", "prev_close", "last_tick_at",
        "last_bucket", "flash_until",
    )
...
class RvolEngine:
    ...
    def _minute_ticker(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            if this_minute != checked_minute:
                ...
                if latest is not None and (self._session is None or latest.date != self._session.date):
    ...
    def _on_tick(self, key: str, vtt: int | None, ltp: float | None, prev_close: float | None) -> None:
        state = self._states.get(key)
        if state is None:
            return
        if vtt is not None:
    ...
    def snapshot(self) -> Snapshot:
        ...
        for state in self._states.values():
            ...
            if bucket != state.last_bucket:
                if ratio is not None and ratio >= self._alert_ratio and state.last_bucket != "unknown":
