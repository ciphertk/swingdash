"""
Wraps MarketDataStreamerV3 and normalises its payloads.

This is the ONLY module that knows the feed's wire shape. Everything
downstream sees plain scalars, so a feed change (or a different broker)
is contained here.

Callbacks run on the SDK's own thread - `connect()` spawns
`ws.run_forever` in a background thread - so anything they touch must be
safe to write from there. They only ever assign scalars on pre-existing
objects, which is atomic under CPython.
"""
from __future__ import annotations

from typing import Callable

import upstox_client

from app.upstox_client_wrapper import get_api_client

# `full` is the only mode carrying `vtt` (volume traded today); `ltpc`
# omits it, so RVOL is impossible without this mode.
_MODE_FULL = "full"

OnTick = Callable[[str, int | None, float | None, float | None], None]
OnStatus = Callable[[str], None]
OnConnection = Callable[[str], None]


class Feed:
    def __init__(
        self,
        instrument_keys: list[str],
        on_tick: OnTick,
        on_status: OnStatus | None = None,
        on_connection: OnConnection | None = None,
    ) -> None:
        self._keys = list(instrument_keys)
        self._on_tick = on_tick
        self._on_status = on_status
        self._on_connection = on_connection
        self._streamer: upstox_client.MarketDataStreamerV3 | None = None

    def start(self) -> None:
        streamer = upstox_client.MarketDataStreamerV3(get_api_client(), self._keys, _MODE_FULL)
        streamer.on("message", self._handle_message)
        streamer.on("open", lambda *_: self._emit_connection("connected"))
        streamer.on("close", lambda *_: self._emit_connection("disconnected"))
        streamer.on("error", lambda *_: self._emit_connection("error"))
        streamer.on("reconnecting", lambda *_: self._emit_connection("reconnecting"))
        streamer.auto_reconnect(True, 5, 10)
        self._streamer = streamer
        streamer.connect()  # non-blocking: runs the socket on its own thread

    def subscribe(self, instrument_keys: list[str]) -> bool:
        """
        Add instruments to the existing connection rather than reconnecting -
        Upstox allows only 2 feed connections per account, so switching
        watchlists must not churn them. False if the socket isn't up yet.
        """
        if not instrument_keys or self._streamer is None:
            return False
        try:
            self._streamer.subscribe(list(instrument_keys), _MODE_FULL)
            return True
        except Exception:
            return False  # socket not open yet; caller retries

    def unsubscribe(self, instrument_keys: list[str]) -> bool:
        if not instrument_keys or self._streamer is None:
            return False
        try:
            self._streamer.unsubscribe(list(instrument_keys))
            return True
        except Exception:
            return False

    def stop(self) -> None:
        if self._streamer is not None:
            try:
                self._streamer.disconnect()
            except Exception:
                pass  # teardown races are noisy on Windows and harmless

    def _emit_connection(self, state: str) -> None:
        if self._on_connection is not None:
            self._on_connection(state)

    def _handle_message(self, message) -> None:
        if not isinstance(message, dict):
            return

        # The feed publishes exchange state changes as their own message
        # type - more authoritative (and free) compared to inferring the
        # session state from the clock.
        if message.get("type") == "market_info":
            status = ((message.get("marketInfo") or {}).get("segmentStatus") or {}).get("NSE_EQ")
            if status and self._on_status is not None:
                self._on_status(status)
            return

        for key, feed in (message.get("feeds") or {}).items():
            market_ff = (feed.get("fullFeed") or {}).get("marketFF")
            if not market_ff:
                continue  # index feeds carry indexFF and have no vtt
            ltpc = market_ff.get("ltpc") or {}
            self._on_tick(
                key,
                _as_int(market_ff.get("vtt")),
                _as_float(ltpc.get("ltp")),
                _as_float(ltpc.get("cp")),
            )


def _as_int(value) -> int | None:
    # vtt arrives as a string in the protobuf payload.
    try:
        return int(value)
    except (TypeError, ValueError):
        return None


def _as_float(value) -> float | None:
    try:
        return float(value)
    except (TypeError, ValueError):
        return None
