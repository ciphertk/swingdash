...
from swingdash.engines.types import DailyBar, MinuteBar
from swingdash.upstox_client_wrapper import fundamentals_api, history_v3_api, market_quote_v3_api
...
def fetch_daily_candles(
    ...
    response: Any = history_v3_api().get_historical_candle_data1(
...
def fetch_todays_minute_candles(instrument_key: str) -> list[MinuteBar]:
    ...
    """
    response: Any = history_v3_api().get_intra_day_candle_data(instrument_key, "minutes", 1)
    return _to_minute_bars(response.data.candles)
...
def fetch_company_profile(isin: str) -> dict:
    ...
    """
    response: Any = fundamentals_api().get_company_profile(isin)
    data = response.data
    return {
        "sector": data.sector,
