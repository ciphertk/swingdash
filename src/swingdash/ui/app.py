...
from swingdash.ui.confirm_modal import ConfirmModal
from swingdash.ui.rvol_table import RvolTable
from swingdash.ui.watchlist_modal import WatchlistModal
...
class RvolApp(App):
    ...
    def __init__(
        self, services: Services, engine: RvolEngine, watchlist_name: str | None = None
    ) -> None:
        super().__init__()
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical():
            with Horizontal(id="toolbar"):
                options = self._watchlist_options()
                initial = next(
    ...
    def action_edit_watchlist(self) -> None:
        ...
        symbols = existing.symbols if existing else ()
        self.push_screen(
    ...
    def action_delete_watchlist(self) -> None:
        ...
        if not name:
            ...
            return
        existing = self.services.watchlists.get(name)
        count = len(existing.symbols) if existing else 0
        self.push_screen(
    ...
    def _confirm_delete(self, confirmed: bool | None) -> None:
        ...
        self.services.watchlists.delete(deleted)
        ...
        self.services.watchlists.set_active(self._watchlist_name)
        self._reload_watchlists()
        self._switch_to(list(remaining[0].symbols) if remaining else [])
    ...
    def _save_watchlist(self, result: tuple[str, str] | None) -> None:
        ...
        self.services.watchlists.save(name, symbols)
        self.services.watchlists.set_active(name)
    ...
    def on_select_changed(self, event: Select.Changed) -> None:
        ...
        if not isinstance(event.value, str) or event.value == self._watchlist_name:
            return
        watchlist = self.services.watchlists.get(event.value)
        if watchlist is None:
            return
        self._watchlist_name = watchlist.name
        self.services.watchlists.set_active(watchlist.name)
        self._switch_to(list(watchlist.symbols))
    ...
    def _watchlist_options(self) -> list[tuple[str, str]]:
        return [(f"{w.name}  ({len(w.symbols)})", w.name) for w in self.services.watchlists.all()]
    ...
    def _market_badge(self, snapshot: Snapshot) -> Text:
        ...
        holiday = self.services.calendar.holiday_for()
